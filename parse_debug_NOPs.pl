...
for my $orig (@all_text_lines) {
	...
	# lsl
		...
		die "lsl needs immediate\n" unless defined $t[2] && $t[2] =~ /^#/;
		$alusrc = 1;
		$imm_signed = imm($t[2]);
		$imm9 = imm9_twos($imm_signed);

		$explain = "lsl: rd=$rd_name rs1=$rs1_name shamt=$imm_signed";
	}
	...
	# ldr
		...
		$explain =
		  "ldr: rd=$rd_name base=$rs1_name ".
		  ($alusrc ? "off=$imm_signed" : "off=0");
	}
	...
	# str (store-data in rs2)
		...
		$explain =
		  "str: store_data=$rs2_name base=$rs1_name ".
		  ($alusrc ? "off=$imm_signed" : "off=0");
	}
...
############################################################
# WRITE OUTPUT FILES (same filenames as before)
############################################################
